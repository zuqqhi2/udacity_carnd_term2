    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
           ...
           *
           */
          double steer_value = 0.0;              // steering to pass simulator
          double throttle_value = 0.1;           // throttle to pass simulator
          const double actuator_delay_sec = 0.1; // simulator's condition

          //=====================================
          // Transform way points to vehicle perspective
          int num_way = ptsx.size();
          Eigen::VectorXd way_x(num_way);
          Eigen::VectorXd way_y(num_way);
          for (int i = 0; i < num_way; i++) {
            double dx = ptsx[i] - px;
            double dy = ptsy[i] - py;
            way_x[i] = dx * cos(-psi) - dy * sin(-psi);
            way_y[i] = dx * sin(-psi) + dy * cos(-psi);
          }
          ...
          int num_estimate_points = 25;
          for (int i = 0; i < num_estimate_points; i++) {
